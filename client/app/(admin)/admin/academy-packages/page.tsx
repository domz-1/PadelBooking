        ...
        <div className="space-y-8 p-8 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage training packages for your coaches.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {coaches.map((coach) => {
                    const filteredPackages = coach.packages?.filter(p =>
                        p.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        coach.User?.name?.toLowerCase().includes(searchTerm.toLowerCase())
                    ) || [];

                    if (searchTerm && filteredPackages.length === 0 && !coach.User?.name?.toLowerCase().includes(searchTerm.toLowerCase())) return null;

                    return (
                        <Card key={coach.id} className="overflow-hidden border-2 bg-background/50 backdrop-blur-sm group hover:border-primary/50 transition-all duration-300">
                            <CardHeader className="bg-muted/30 pb-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <CardTitle className="text-lg">{coach.User?.name}</CardTitle>
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                                                Professional Coach
                                            </p>
                                        </div>
                                    </div>
                                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => handleOpenDialog(undefined, coach.id)}>
                                ...
                                <div className="divide-y divide-border/50">
                                    {coach.packages && coach.packages.length > 0 ? (
                                        coach.packages.map((pkg) => (
                                            <div key={pkg.id} className="p-6 hover:bg-muted/20 transition-colors">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div>
                                                        <h4 className="font-bold text-sm">{pkg.name}</h4>
                                                        <div className="flex items-center gap-1.5 text-primary mt-1">
                                                            <DollarSign className="h-3 w-3" />
                                                            <span className="font-black text-sm">{pkg.price.toLocaleString()} EGP</span>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleOpenDialog(pkg)}>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>{editingPackage ? "Edit Package" : "Add Academy Package"}</DialogTitle>
                        <DialogDescription>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="space-y-2">
                            <label className="text-xs font-bold uppercase tracking-widest opacity-70">Select Coach</label>
                            <Select
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold uppercase tracking-widest opacity-70">Package Name</label>
                            <Input
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold uppercase tracking-widest opacity-70">Price (EGP)</label>
                            <Input
                                type="number"
                                placeholder="0.00"
                                value={form.price}
