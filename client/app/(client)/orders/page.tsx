...
export default function MyOrdersPage() {
    ...
    if (loading) return <div className="container mx-auto p-12 text-center">Loading orders...</div>;
            ...
            <div className="container mx-auto px-4 max-w-4xl">
                ...
                {orders.length === 0 ? (
                    <div className="bg-card rounded-[2.5rem] p-12 text-center border">
                        ...
                        <a href="/store" className="text-primary font-bold hover:underline">Start Shopping</a>
                    </div>
                ) : (
                    <div className="space-y-8">
                        {orders.map((order) => (
                            <Card key={order.id} className="rounded-[2.5rem] overflow-hidden border-none shadow-xl shadow-zinc-200/50">
                                        ...
                                        {order.OrderItems.map((item) => (
                                            <div key={item.id} className="flex items-center gap-6 group">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-muted-foreground">Qty: {item.quantity} × {item.price} EGP</p>
                                                </div>
                                    ...
                                    <div className="mt-8 pt-8 border-t flex flex-wrap justify-between items-center gap-4">
                                        <div className="flex items-center gap-4 bg-muted/30 px-4 py-2 rounded-2xl">
                                            ...
                                            <div className="text-xs">
                                                <p className="text-muted-foreground uppercase font-bold tracking-tighter">Payment Method</p>
                                                <p className="font-black capitalize">{order.paymentMethod}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-sm text-muted-foreground font-medium">Total Amount</p>
                                            <p className="text-3xl font-black text-primary">{Number(order.totalAmount).toLocaleString()} <span className="text-sm">EGP</span></p>
                                        </div>
                                    </div>
                                </div>
                            </Card>
